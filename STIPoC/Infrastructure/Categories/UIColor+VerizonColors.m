//
//  UIColor+VerizonColors.m
//  STIPoC
//
//  Created by Alex Gutierrez on 11/30/13.
//  Copyright (c) 2013 Administrator. All rights reserved.
//

#import "UIColor+VerizonColors.h"

NSString *const kSTIPoCFontHelveticaNeueLight = @"HelveticaNeue-Light";
NSString *const kSTIPoCFontHelveticaNeueMedium = @"HelveticaNeue-Medium";

@implementation UIColor (VerizonColors)

+ (UIColor *)verizonDarkRed
{
    return UIColorFromRGB(0xAC0404);
}

+ (UIColor *)verizonRed
{
    return UIColorFromRGB(0xed1c24);
}

+ (UIColor *)verizonBlack
{
    return UIColorFromRGB(0x333333);
}

+ (UIColor *)verizonGrey
{
    return UIColorFromRGB(0x595a5d);
}

+ (UIColor *)verizonDarkGrey
{
    return UIColorFromRGB(0xd2d3d4);
}

+ (UIColor *)verizonLightGrey
{
    return UIColorFromRGB(0xecedee);
}

+ (UIColor *)verizonTurquoise
{
    return UIColorFromRGB(0x73b9b0);
}

+ (UIColor *)verizonGreen
{
    return UIColorFromRGB(0x4a9a4d);
}

+ (UIColor *)verizonYellow
{
    return UIColorFromRGB(0xfadf4c);
}

+ (UIColor *)verizonBlue
{
    return UIColorFromRGB(0x2e67b2);
}

+ (UIColor *)verizonBeige
{
    return UIColorFromRGB(0xc49f39);
}

+ (UIColor *)verizonFucsia
{
    return UIColorFromRGB(0xd2669f);
}

+ (UIColor *)verizonPurple
{
    return UIColorFromRGB(0x7f3483);
}

+ (UIColor *)verizonLightBlue
{
    return UIColorFromRGB(0x3195a9);
}
@end
